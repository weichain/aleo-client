...
const Component = () => {
  ...
  const handleMultiMethodSubmit = async (event: MouseEvent<HTMLButtonElement, globalThis.MouseEvent>) => {
    ...
    const privateKeyObject = aleo.PrivateKey.from_string("APrivateKey1zkp2J6XPd4EgLbSkEHccUGaqgTGjbgEgVdnDdvk6L7CdG81" as string);
    ...
    if (transitions[1]){
      const secondInputAddresses = JSON.stringify(createAddressesInput(_recipients.slice(numberPerTransition[0] + 1))).replaceAll('"','');
      const secondInputAmounts = JSON.stringify(createAmountsInput(_amounts.slice(numberPerTransition[0] + 1))).replaceAll('"','');
